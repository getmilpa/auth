...
final class WebAuthnCredentialRecordTest extends TestCase
{
    ...
    public function testTouchStampsLastUsedWithoutMutatingTheOriginal(): void
    {
        $r = $this->record();
        self::assertNull($r->lastUsedAt);
        $now = new \DateTimeImmutable('2026-01-02T00:00:00Z');
        $r2 = $r->touch($now);
        self::assertNull($r->lastUsedAt);
        self::assertEquals($now, $r2->lastUsedAt);
        self::assertSame('cred-b64', $r2->credentialId);
        self::assertSame(5, $r2->signCount);
    }
}
